export function jsonToBpmn(data: any): string {
    ...
    const isEnd = (id: string) =>
      endEvents.includes(id) || (!tasks.some((t: any) => t.id === id) && !isDecision(id));
    ...
    Object.entries(byDepth).forEach(([col, ids]) => {
      ...
      ids.forEach((id, row) => {
        ...
        if (id === startId) {
          positions[id] = { x, y: y + TASK_H / 2 - EVENT_R, w: EVENT_R * 2, h: EVENT_R * 2 };
        } else if (isEnd(id)) {
          positions[id] = { x, y: y + TASK_H / 2 - EVENT_R, w: EVENT_R * 2, h: EVENT_R * 2 };
        } else if (isDecision(id)) {
          positions[id] = { x, y: y + TASK_H / 2 - GW_SIZE / 2, w: GW_SIZE, h: GW_SIZE };
        } else {
    ...
    connections.forEach((c: any) => {
      ...
      if (!tasks.some((t: any) => t.id === id) && !isDecision(id) && id !== startId) {
        endEventIds.add(id);
      }
    });
  
    // End event names
    const endName = (id: string) => id.replace(/_/g, " ").replace(/\b\w/g, (l) => l.toUpperCase());
    ...
    const processXml = `
      ...
      ${tasks
        .map((t: any) => {
          const incoming = connections.filter((c: any) => c.to_id === t.id).map((_: any, i: number) => `<bpmn:incoming>flow_${t.id}_in_${i}</bpmn:incoming>`).join("\n");
          const outgoing = connections.filter((c: any) => c.from_id === t.id).map((_: any, i: number) => `<bpmn:outgoing>flow_${t.id}_out_${i}</bpmn:outgoing>`).join("\n");
          const tag = t.type === "system" ? "bpmn:serviceTask" : "bpmn:userTask";
      ...
      ${decisions
        .map((d: any) => {
          const incoming = connections.filter((c: any) => c.to_id === d.id).map((_: any, i: number) => `<bpmn:incoming>flow_${d.id}_in_${i}</bpmn:incoming>`).join("\n");
          const outgoing = connections.filter((c: any) => c.from_id === d.id).map((_: any, i: number) => `<bpmn:outgoing>flow_${d.id}_out_${i}</bpmn:outgoing>`).join("\n");
      ...
      ${Array.from(endEventIds)
        .map((id) => {
          const incoming = connections.filter((c: any) => c.to_id === id).map((_: any, i: number) => `<bpmn:incoming>flow_${id}_in_${i}</bpmn:incoming>`).join("\n");
      ...
      ${connections
        .map((c: any) => {
          // build unique id
          const fromOutIdx = connections.filter((x: any) => x.from_id === c.from_id).indexOf(c);
          const toInIdx = connections.filter((x: any) => x.to_id === c.to_id).indexOf(c);
