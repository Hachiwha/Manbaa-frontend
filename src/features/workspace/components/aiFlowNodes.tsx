...
export const RfStartNode = memo(
  ({ data, selected }: NodeProps<RfFlowNodeData>) => (
    <div
    ...
    >
      ...
      <span className="max-w-[160px] truncate text-[9px] text-muted-foreground">
        {data.subtitle}
      </span>
      <Handle
        type="source"
        position={Position.Bottom}
        className="!h-2.5 !w-2.5 !border-background !bg-success"
      />
    </div>
  ),
);
...
export const RfEndNode = memo(
  ({ data, selected }: NodeProps<RfFlowNodeData>) => (
    ...
    >
      ...
      <span className="text-[11px] font-semibold">{data.title}</span>
      <span className="max-w-[140px] truncate text-center text-[9px] text-muted-foreground">
        {data.subtitle}
      </span>
    </div>
  ),
  ...
  ({ data, selected }: NodeProps<RfFlowNodeData>) => (
    ...
    >
      <Handle
        type="target"
        position={Position.Top}
        className="!h-2 !w-2 !border-background !bg-warning"
      />
...
export const RfTaskNode = memo(
  ({ data, selected }: NodeProps<RfFlowNodeData>) => {
    ...
    return (
      <div
        className={`relative w-[260px] rounded-xl border-2 bg-card/95 p-3 backdrop-blur transition-all ${ring}`}
      >
        <Handle
          type="target"
          position={Position.Top}
          ...
          <div
            className={`grid h-9 w-9 shrink-0 place-items-center rounded-lg ${
              human
                ...
                : "bg-surface-3 text-ink-muted-80"
            }`}
          >
...
export const RfDecisionNode = memo(
  ({ data, selected }: NodeProps<RfFlowNodeData>) => {
