...
const Icons = {
  ...
  Flow: () => (
    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <path d="M9 6h3a3 3 0 013 3v3" /><polyline points="15 12 18 15 15 18" />
    </svg>
  ),
};
...
function RouteComponent() {
  ...
  const handleGenerate = () => {
    ...
    setTimeout(() => {
      try {
        ...
        setMeta({ tasks: t.length, gateways: d.length, flows: c.length, ends: e.length });
  ...
  const handleDownloadXML = () => { download(new Blob([xml], { type: "application/xml" }), "diagram.bpmn"); showToast(".bpmn saved"); };
  ...
  const handleExportPDF = async () => {
    ...
    if (!w) { const vb = (svgEl.getAttribute("viewBox") || "0 0 1200 600").split(/[\s,]+/); w = +vb[2] || 1200; h = +vb[3] || 600; }
    const url = URL.createObjectURL(new Blob([svg], { type: "image/svg+xml" }));
    await new Promise<void>((res, rej) => {
      const img = new Image();
      img.onload = () => {
        const sc = 2; const cv = document.createElement("canvas");
  ...
  const handleClear = () => {
    setJsonInput(""); setXml(""); setError(null); setMeta(null);
    try { viewerRef.current?.clear(); } catch {}
  };

  // ─── Pill button ─────────────────────────────────────────────────────────
  const PillBtn = ({ onClick, disabled, children, color = "#6366f1", outline = false }: any) => (
    <button onClick={onClick} disabled={disabled} style={{
      ...
      onMouseOver={e => { if (!disabled) (e.currentTarget as HTMLElement).style.opacity = "0.82"; }}
      onMouseOut={e => { if (!disabled) (e.currentTarget as HTMLElement).style.opacity = "1"; }}
    >
      {children}
    </button>
  );
  ...
  const ZoomBtn = ({ onClick, title, children }: any) => (
    <button onClick={onClick} title={title} style={{
      ...
      boxShadow: "0 1px 4px #00000010", transition: "all 0.15s",
    }}
      onMouseOver={e => { const el = e.currentTarget as HTMLElement; el.style.color = "#6366f1"; el.style.borderColor = "#a5b4fc"; }}
      onMouseOut={e => { const el = e.currentTarget as HTMLElement; el.style.color = "#64748b"; el.style.borderColor = "#e2e8f0"; }}
    >
      {children}
    </button>
  );
  ...
  return (
    <div style={{ fontFamily: "'IBM Plex Mono', monospace", height: "100vh", display: "flex", flexDirection: "column", background: "#0d1117", overflow: "hidden" }}>
      ...
      <header style={{
        ...
        <span style={{ fontSize: 13, fontWeight: 700, color: "#c7d2fe", letterSpacing: "0.08em" }}>
        ...
        {toast && (
          <div style={{
            display: "flex", alignItems: "center", gap: 6,
            background: "#0f2818", border: "1px solid #166534",
            borderRadius: 6, padding: "4px 11px", fontSize: 11.5, color: "#86efac",
          }}>
            <Icons.Check /> {toast}
          </div>
        )}
        ...
        {xml && (
          <div style={{ display: "flex", gap: 6 }}>
            <PillBtn onClick={handleDownloadXML} outline color="#10b981"><Icons.Download /> BPMN</PillBtn>
            <PillBtn onClick={handleExportSVG} outline color="#8b5cf6"><Icons.Image /> SVG</PillBtn>
            <PillBtn onClick={handleExportPDF} outline color="#ef4444"><Icons.File /> PDF</PillBtn>
          </div>
        )}
      </header>
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        ...
        <aside style={{
          ...
          <div style={{ display: "flex", borderBottom: "1px solid #1e2533", flexShrink: 0 }}>
            {(["json", "xml"] as const).map(t => (
              <button key={t} onClick={() => setTab(t)} style={{
                ...
                color: tab === t ? "#a5b4fc" : "#475569", cursor: "pointer",
          ...
          {tab === "json" && (
            <div style={{
              ...
              <button onClick={() => { setJsonInput(SAMPLE_JSON); setError(null); }} style={{
                fontSize: 11, padding: "3px 10px", background: "#1a2535",
                border: "1px solid #2d3d55", color: "#7dd3fc", borderRadius: 5, cursor: "pointer",
              }}>Load sample</button>
              <button onClick={handleClear} style={{
                fontSize: 11, padding: "3px 10px", background: "transparent",
                border: "1px solid #1e2533", color: "#475569", borderRadius: 5, cursor: "pointer",
          ...
          {tab === "json" ? (
            <textarea
              ...
              style={{
                ...
                background: "#090d13", color: "#93c5fd",
                fontFamily: "'IBM Plex Mono', monospace", fontSize: 12, lineHeight: 1.75,
                padding: "14px 14px",
              }}
            />
          ) : (
            <pre style={{
              ...
              background: "#090d13", color: "#6ee7b7",
          ...
          {error && (
            <div style={{
              flexShrink: 0,
              padding: "10px 14px", background: "#160a0a",
          ...
          {meta && (
            <div style={{
              ...
              background: "#0a1020", borderTop: "1px solid #1e2533",
            }}>
              <div style={{ fontSize: 10, color: "#334155", letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 8, fontWeight: 700 }}>
                Diagram Summary
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "5px 0" }}>
                {[
                  ...
                  <div key={label as string} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12 }}>
                    <span style={{ width: 6, height: 6, borderRadius: "50%", background: color as string, flexShrink: 0 }} />
                    <span style={{ color: "#475569" }}>{label}</span>
                    <span style={{ marginLeft: "auto", color: color as string, fontWeight: 700, paddingRight: 8 }}>{val}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <button onClick={handleGenerate} disabled={loading} style={{
            flexShrink: 0,
            margin: "12px", padding: "11px 0",
            background: loading ? "#1e2533" : "linear-gradient(135deg,#6366f1 0%,#8b5cf6 100%)",
            border: "none", borderRadius: 9,
            color: loading ? "#475569" : "#fff",
            fontFamily: "'IBM Plex Mono', monospace",
            fontWeight: 700, fontSize: 12.5, letterSpacing: "0.08em", textTransform: "uppercase",
        ...
        {/* ══ RIGHT PANEL: VIEWER ══ */}
        <main style={{ flex: 1, position: "relative", background: "#f1f5f9", overflow: "hidden" }}>
          ...
          <div style={{
            ...
            <ZoomBtn onClick={() => zoom("in")} title="Zoom in"><Icons.ZoomIn /></ZoomBtn>
            <ZoomBtn onClick={() => zoom("out")} title="Zoom out"><Icons.ZoomOut /></ZoomBtn>
            <ZoomBtn onClick={() => zoom("fit")} title="Fit viewport"><Icons.Fit /></ZoomBtn>
          </div>
          ...
          {!xml && (
            <div style={{
              ...
              <div style={{ textAlign: "center" }}>
                <p style={{ margin: 0, fontSize: 15, fontWeight: 700, color: "#475569", letterSpacing: "0.02em" }}>
                  No diagram yet
                </p>
                <p style={{ margin: "6px 0 0", fontSize: 12.5, color: "#94a3b8", lineHeight: 1.6 }}>
                  Paste your JSON and click <br />
                  <strong style={{ color: "#6366f1" }}>Generate BPMN</strong> to render the diagram
                </p>
              </div>
            </div>
          )}
          ...
          {xml && (
            <div style={{
              ...
              background: "rgba(15,20,30,0.78)", backdropFilter: "blur(8px)",
